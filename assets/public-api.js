  ...
  if (availability) {
    ...
    getClasses().then(payload => {
      ...
      availability.innerHTML = open.map((item, index) => {
        ...
        return `<article class="availability-card reveal visible" style="--reveal-delay:${Math.min(index * 60, 240)}ms"><span class="class-day">${esc(days[item.weekday])} · ${esc(time)}</span><h3>${esc(item.title)}</h3><p>${esc(item.level)} · ${esc(item.duration_minutes)} minutes<br>${esc(item.location_name)}</p><div class="availability-card-foot"><div><strong>${item.available} ${item.available === 1 ? 'place' : 'places'} showing</strong><span>${/private/i.test(item.title)?'Contact us for private pricing':`${new Intl.NumberFormat('en-AU',{style:'currency',currency:'AUD'}).format(Number(item.price || 0))} per lesson · confirmed before enrolment`}</span></div><a class="text-link" href="enquire.html?${query}">Enquire</a></div></article>`;
      }).join('') || '<div class="empty-state"><strong>No classes are showing places right now.</strong><p>Places can reopen as families change days. Send an enquiry and the team will check the current timetable for you.</p><a class="btn btn-blue btn-small" href="enquire.html">Ask about availability <span aria-hidden="true">&rarr;</span></a></div>';
      document.getElementById('availability-source').textContent = term.configured&&term.in_session?'Current in-term places are shown here. Standard lessons are $22.50 each, billed by the term and due on enrolment. Private pricing and class fit are confirmed personally.':term.configured?'Today is outside the published term. Send an enquiry and the team will confirm the next enrolment window.':'Management term dates have not been published yet. Send an enquiry and the team will confirm the next enrolment window.';
    }).catch(() => {
