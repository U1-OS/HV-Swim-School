...
def test_login_budgets_survive_ip_rotation_and_account_spraying(client):
    ...
    try:
        for index in range(server.LOGIN_ACCOUNT_LIMIT):
            server.debit_login_attempt(email, f"192.0.2.{index}")
        with pytest.raises(Exception) as error:
            server.debit_login_attempt(email, "192.0.2.250")
        assert error.value.status_code == 429
        assert error.value.headers["Retry-After"] == "900"
        for index in range(server.LOGIN_IP_LIMIT):
            server.debit_login_attempt(f"spray{index}@example.com", "198.51.100.1")
        with pytest.raises(Exception) as error:
            server.debit_login_attempt("new-account@example.com", "198.51.100.1")
        assert error.value.status_code == 429
    finally:
        with db_session() as db:
            db.execute("DELETE FROM login_attempts")
