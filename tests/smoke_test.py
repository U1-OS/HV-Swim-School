...
def main():
    ...
    expect("health endpoint", status == 200 and health.get("version") == "5.0.0")
    ...
    expect("native mobile launch shell", "Open connected app" in mobile_shell and "V5.0" in mobile_shell)
    ...
    for role, (email, password, protected_path, result_key) in roles.items():
        ...
        if role == "admin":
            ...
            expect("admin merchandise workspace", status == 200 and len(merch.get("catalogue", [])) >= 9)
