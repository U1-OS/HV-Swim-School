...
def main():
    ...
    expect("fresh public conditions", 'data-weather-value' in home_page and 'data-pool-value' in home_page and 'data-venue-status' in home_page)
    about_page = public.open(BASE + "/about.html", timeout=30).read().decode("utf-8")
    expect("public care and program routes", 'child-safety.html' in about_page and 'programs.html' in about_page and 'assets/site.css' in about_page)
    programs_page = public.open(BASE + "/programs.html", timeout=30).read().decode("utf-8")
    expect("program matching and pricing", 'program-matcher' in programs_page and 'program-availability' in programs_page and '$22.50 per lesson' in programs_page)
    ...
    expect("gated collection and basket", 'shop-load-more' in shop_page and 'cart-drawer' in shop_page and 'ordering unavailable' in shop_script)
    expect("no retired supplier demo", 'data-scene="bottle"' not in shop_page and 'production-route-grid' not in shop_page)
    enquire_page = public.open(BASE + "/enquire.html", timeout=30).read().decode("utf-8")
    expect("guided enquiry safe fallback", 'wizard-class-grid' in enquire_page and 'method="post" action="/api/public/enquiries"' in enquire_page and 'enrolment-success' in enquire_page)
