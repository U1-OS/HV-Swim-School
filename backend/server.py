...
# The dummy verifier prevents an unknown email taking a cheaper password path.
DUMMY_PASSWORD_HASH = password_hash(new_token(32))
LOGIN_ACCOUNT_LIMIT = 10
LOGIN_IP_LIMIT = 50


def debit_login_attempt(email: str, ip: str) -> int:
    cutoff = (datetime.now(timezone.utc) - timedelta(minutes=15)).isoformat()
    # Reserve and commit before checking credentials: failures and concurrent requests
    # cannot roll back their debit or race past the limit.
    with db_session() as db:
        db.execute("BEGIN IMMEDIATE")
        db.execute("DELETE FROM login_attempts WHERE created_at<=?", (login_attempt_cutoff(),))
        account_failures = db.execute("SELECT COUNT(*) FROM login_attempts WHERE email=? AND success=0 AND created_at>?", (email, cutoff)).fetchone()[0]
        ip_failures = db.execute("SELECT COUNT(*) FROM login_attempts WHERE ip_address=? AND success=0 AND created_at>?", (ip, cutoff)).fetchone()[0]
        if account_failures >= LOGIN_ACCOUNT_LIMIT or ip_failures >= LOGIN_IP_LIMIT:
            raise HTTPException(429, "Too many sign-in attempts. Try again in 15 minutes.", headers={"Retry-After": "900"})
        return db.execute("INSERT INTO login_attempts(email,ip_address,success,created_at) VALUES(?,?,0,?)", (email, ip, now_iso())).lastrowid
...
def login(payload: LoginInput, request: Request, response: Response) -> dict[str, Any]:
    ...
    attempt_id = debit_login_attempt(email, ip)
    with db_session() as db:
        db.execute("BEGIN IMMEDIATE")
        user = db.execute("SELECT * FROM users WHERE email=? AND active=1", (email,)).fetchone()
        password_ok = password_verify(payload.password, user["password_hash"] if user else DUMMY_PASSWORD_HASH)
        success = bool(user and password_ok and not (settings.production and email.endswith("@hvswim.demo")) and identity.verify_mfa(db, user, payload.code))
        if not success:
            raise HTTPException(status_code=401, detail="Sign-in details are incorrect. Check your authenticator code if enabled.")
        db.execute("UPDATE login_attempts SET success=1 WHERE id=?", (attempt_id,))
